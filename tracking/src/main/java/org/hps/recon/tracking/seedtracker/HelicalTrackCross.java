...
public class HelicalTrackCross extends org.lcsim.fit.helicaltrack.HelicalTrackCross {
    ...
    public void init(HelicalTrackStrip strip1, HelicalTrackStrip strip2) {
        init(HitUtils.PositionFromOrigin(strip1, strip2), HitUtils.CovarianceFromOrigin(strip1, strip2), strip1.dEdx() + strip2.dEdx(), 0.5 * (strip1.time() + strip2.time()), _type, null, strip1.detector(), strip1.layer(), strip1.BarrelEndcapFlag());
        _strip1 = strip1;
        _strip2 = strip2;
        _helix = null;
        init();
    ...
    public void setTrackDirection(TrackDirection trkdir, SymmetricMatrix hcov) {
        ...
        if (errok) {
            ...
            System.out.println("failed errok");
            resetTrackDirection();
        }
        return;
    }
    ...
    private double ChisqPenalty(TrackDirection trkdir, SymmetricMatrix hcov) {
        ...
        if (v1 < _strip1.vmin())
            chisq += Math.pow((v1 - _strip1.vmin()) / dv1, 2);
        if (v1 > _strip1.vmax())
            chisq += Math.pow((v1 - _strip1.vmax()) / dv1, 2);
        if (v2 < _strip2.vmin())
            chisq += Math.pow((v2 - _strip2.vmin()) / dv2, 2);
        if (v2 > _strip2.vmax())
            chisq += Math.pow((v2 - _strip2.vmax()) / dv2, 2);
        return chisq;
    }
