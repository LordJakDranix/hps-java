...
import org.lcsim.recon.tracking.seedtracker.MultipleScattering;
...
public class HelixFitter extends org.lcsim.recon.tracking.seedtracker.HelixFitter {
    ...
    public boolean FitCandidate(SeedCandidate seed, SeedStrategy strategy) {
        if (_debug)
            System.out.printf("%s: FitCandidate() with stategy: \"%s\"\n", this.getClass().getSimpleName(), strategy.getName());
        ...
        if (_debug) {
            ...
            for (HelicalTrackHit hit : hitlist) {
                System.out.printf("%s: (%.2f,%.2f,%.2f) corrected  %s\n", this.getClass().getSimpleName(), hit.getPosition()[0], hit.getPosition()[1], hit.getPosition()[2], hit.getCorrectedPosition().toString());
        ...
        if (oldhelix == null) {
            ...
            if (_status != FitStatus.Success) {
                if (_diag != null)
                    _diag.fireHelixFitFailed(seed, _status, true);
                return false;
            }
        ...
        if (_status != FitStatus.Success) {
            if (_diag != null)
                _diag.fireHelixFitFailed(seed, _status, false);
            return false;
        }
    ...
    private void SaveFit() {
        ...
        if (_status == FitStatus.CircleFitFailed)
            return;
        ...
        if (_status == FitStatus.InconsistentSeed)
            return;
        if (_status == FitStatus.LineFitFailed)
            return;
        if (_status == FitStatus.ZSegmentFitFailed)
            return;
